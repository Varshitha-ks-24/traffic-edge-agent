import { c as createLucideIcon, T as TrafficLevel } from "./index-Cjz6nUaB.js";
/**
...
const DISTRICT_ADJACENCY = [
  ...
  ["Downtown Core", "Southport Harbor", 4],
...
function buildGraph(segments) {
  const districtCongestion = /* @__PURE__ */ new Map();
  const districtCount = /* @__PURE__ */ new Map();
...
function dijkstra(graph, from, to) {
  ...
  const rawDist = /* @__PURE__ */ new Map();
...
function generateReasoning(path, totalDist, estimatedMinutes, segments, from, to) {
  const districtCongestion = /* @__PURE__ */ new Map();
  const districtCount = /* @__PURE__ */ new Map();
  for (const seg of segments) {
    districtCongestion.set(
  ...
  const avgForDistrict = (d) => {
    const total = districtCongestion.get(d) ?? 0;
    const count = districtCount.get(d) ?? 1;
    return total / count;
  };
  const overallCong = path.reduce((acc, d) => acc + avgForDistrict(d), 0) / path.length;
  const congDesc = overallCong >= 70 ? "heavy" : overallCong >= 40 ? "moderate" : "light";
  ...
  const avoidedStr = avoidedDistricts.length > 0 ? ` Congested zones ${avoidedDistricts.slice(0, 2).join(" and ")} were avoided.` : "";
...
function computeLocalOptimalRoute(from, to, segments = FALLBACK_SEGMENTS) {
  ...
  if (!result) {
    ...
    const waypoints = [(fromSeg == null ? void 0 : fromSeg.name) ?? from, (toSeg == null ? void 0 : toSeg.name) ?? to];
    const dist = 6.5;
  ...
  const waypointNames = path.map((district) => {
    const segsForDistrict = segments.filter((s) => s.district === district);
    if (!segsForDistrict.length) return district;
    const best = segsForDistrict.reduce(
      (a, b) => Number(a.congestionPct) <= Number(b.congestionPct) ? a : b
    );
    return best.name;
  });
  const avgCong = path.reduce((acc, d) => {
    const segs = segments.filter((s) => s.district === d);
    if (!segs.length) return acc + 40;
    const avg = segs.reduce((s, seg) => s + Number(seg.congestionPct), 0) / segs.length;
    return acc + avg;
  }, 0) / path.length;
...
async function computeOptimalRoute(actor, from, to, segments = FALLBACK_SEGMENTS) {
  if (actor) {
    try {
...
const VEHICLE_DESCRIPTIONS = {
  Ambulance: (s) => `Emergency medical unit responding to a critical incident on ${s}. All cross-traffic suspended. AI-guided corridor cleared for priority passage.`,
  "Fire Truck": (s) => `Fire apparatus dispatched to active emergency on ${s}. Signal preemption active. Intersection clearance in progress along response corridor.`,
  Police: (s) => `Law enforcement unit responding at high speed through ${s}. Traffic control adjusted. Bystanders advised to yield immediately.`,
  Hazmat: (s) => `Hazardous materials vehicle detected on ${s}. Maximum priority corridor engaged. Civilian traffic rerouted to secondary streets.`
};
const VEHICLE_ACTIONS = {
  Ambulance: "Clear all lanes on primary corridor immediately. Activate preemptive green wave. Dispatch traffic control officers to manual override positions.",
  "Fire Truck": "Suspend all opposing signals for 90 seconds. Broadcast public alert via city PA system. Prepare secondary access route as fallback.",
  Police: "Enable dynamic signal override mode. Restrict civilian access to affected blocks. Coordinate with central dispatch for route clearance.",
  Hazmat: "Activate full perimeter containment protocol. Reroute all civilian traffic minimum 3 blocks. Alert hazmat response team for standing by."
};
function createLocalEmergencyAlert(segmentId, vehicleType, segmentName) {
  const name = segmentName ?? segmentId;
  const priority = VEHICLE_PRIORITIES[vehicleType] ?? 7;
  const descFn = VEHICLE_DESCRIPTIONS[vehicleType] ?? ((s) => `Emergency vehicle deployed to ${s}. Priority corridor active.`);
  const action = VEHICLE_ACTIONS[vehicleType] ?? "Clear affected corridor and coordinate with emergency services immediately.";
  return {
